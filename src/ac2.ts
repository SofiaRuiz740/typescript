...
const listaDeEmpleados: Empleado[] = [] as const;

const agregarNuevoEmpleado = (empleadoNuevo: Empleado): void => {
...
const buscarPorNombre = (nombreBuscado: string): Empleado[] => {
...
const calcularPromedioSalarios = (): number | never => {
...
const iniciarSistemaEmpleados = (): void => {
  while (true) {
    const opcion = prompt(
      ...
      case "2":
        ...
        if (encontrados.length > 0) {
          encontrados.forEach(emp =>
            alert(`ID: ${emp.id}, Nombre: ${emp.nombre}, Salario: ${emp.salario}`)
          );
        }
        break;
